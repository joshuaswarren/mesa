// Generic Vulkan compute runner for precise-math reproducers.
// usage: mathrepro <shader.spv> <in.bin> <out.bin> <out_bytes> <groups> [reps]
// binding 0: input SSBO (file contents), binding 1: output SSBO (out_bytes).
// Dispatches <groups> workgroups; prints device name and GPU time (ms) per
// dispatch when reps > 1.
...
int main(int argc, char** argv) {
  ...
  int reps = argc > 6 ? std::atoi(argv[6]) : 1;
  ...
  if (reps > 1) {
    ...
    std::printf("timing_ms median=%.3f min=%.3f max=%.3f reps=%d\n",
                ms[ms.size() / 2], ms.front(), ms.back(), reps);
  }
