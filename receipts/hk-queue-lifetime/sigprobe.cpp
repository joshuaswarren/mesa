...
int main(int argc, char** argv) {
  ...
  // Multi-signal test. nsig fresh timeline semaphores signaled by one submit:
  // sem[i] at value vals[i]; sem order as given. Report each counter after
  // the job is known complete (via the LAST semaphore in the list).
  uint32_t nsig = argc > 5 ? std::strtoul(argv[5], nullptr, 10) : 2;
  std::vector<VkSemaphore> sems(nsig);
  std::vector<uint64_t> vals(nsig);
  for (uint32_t i = 0; i < nsig; ++i) {
    CK(vkCreateSemaphore(dev, &sci, nullptr, &sems[i]));
    vals[i] = argc > 6 + (int)i ? std::strtoull(argv[6 + i], nullptr, 10) : 1;
  }
  VkTimelineSemaphoreSubmitInfo tsi{VK_STRUCTURE_TYPE_TIMELINE_SEMAPHORE_SUBMIT_INFO};
  tsi.signalSemaphoreValueCount = nsig;
  tsi.pSignalSemaphoreValues = vals.data();
  VkSubmitInfo si{VK_STRUCTURE_TYPE_SUBMIT_INFO};
  si.pNext = &tsi;
  si.commandBufferCount = 1;
  si.pCommandBuffers = &cb;
  si.signalSemaphoreCount = nsig;
  si.pSignalSemaphores = sems.data();
  auto t0 = std::chrono::steady_clock::now();
  CK(vkQueueSubmit(queue, 1, &si, VK_NULL_HANDLE));
  auto ms = [&] {
    return std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count();
  };
  // Wait on every semaphore individually with a bounded timeout, report.
  int bad = 0;
  for (uint32_t i = 0; i < nsig; ++i) {
    VkSemaphoreWaitInfo wi{VK_STRUCTURE_TYPE_SEMAPHORE_WAIT_INFO};
    wi.semaphoreCount = 1;
    wi.pSemaphores = &sems[i];
    wi.pValues = &vals[i];
    VkResult wr = vkWaitSemaphores(dev, &wi, 5ull * 1000 * 1000 * 1000);
    uint64_t cur = 0;
    CK(vkGetSemaphoreCounterValue(dev, sems[i], &cur));
    std::printf("signal[%u] target=%llu wait=%s counter=%llu at %.1f ms\n", i,
                (unsigned long long)vals[i],
                wr == VK_SUCCESS ? "OK" : wr == VK_TIMEOUT ? "TIMEOUT" : "ERR",
                (unsigned long long)cur, ms());
    if (wr != VK_SUCCESS || cur < vals[i]) bad++;
  }
  std::printf("result[0]=%g\n", host[0]);
  std::printf("VERDICT: %s\n", bad ? "SIGNAL_DROPPED" : "ALL_SIGNALED");
